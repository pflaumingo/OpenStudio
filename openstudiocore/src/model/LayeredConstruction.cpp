...
bool LayeredConstruction::layersAreValid(const std::vector<FenestrationMaterial>& fenestrationMaterials)
{
  ...
  for (const auto & fenestrationMaterial : fenestrationMaterials) {
    bool isGlazing = bool(fenestrationMaterial.optionalCast<Glazing>());
    bool isSimpleGlazing = bool(fenestrationMaterial.optionalCast<SimpleGlazing>());
    bool isGasLayer = bool(fenestrationMaterial.optionalCast<GasLayer>());
    // Rule 1
    if (isSimpleGlazing) {
